...
static ssize_t read_func(struct file* filp, char __user *buf,size_t s,loff_t* t)
{
  ...
  k_buf = (int*)i;
  // True random numbers are generated by reading random device files
  // which generates random numbers system log files as seed(entropy).
  fp = filp_open("/dev/random",O_NONBLOCK,0);
  if(fp==NULL)
  {
    printk(KERN_INFO"Couldn't open the /dev/random file");
    return -EFAULT;
  }
  if(!(fp->f_op->read(fp,i,4,&fp->f_pos)))
  {
    printk(KERN_INFO"Couldn't read the /dev/random file");
    return -EFAULT;
  }
  *k_buf = *k_buf&(0x03ff); // Making it 10 bit.
   if(copy_to_user(buf,k_buf,4))
   {
